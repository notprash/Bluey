...
class Anime(commands.Cog):
    ...
    async def anime(self, ctx, *query):
        ...
        if data['count'] == 0:
            emb = discord.Embed(title='Anime', description= f'Sorry! No data on {query} found.', color = discord.Color.red())
    ...
    async def manga(self, ctx, *query):
        query = ' '.join(query)
        data = kitsupy.search('manga', query)
        if data['count'] == 0:
            emb = discord.Embed(title='Manga', description= f'Sorry! No data on {query} found.', color = discord.Color.red())
            await ctx.send(embed=emb)
        else:
            id = getlist(data)
            result = kitsupy.get_info('manga', id[1])
            en_title, jp_title = result['titles']['en'], result['titles']['ja_jp']
            en_jp_title = ''
            if result['titles']['en_jp'] == '':
                en_jp_title = en_title
            ...
            emb.add_field(name='Start Date', value=start_date, inline=True)
            emb.add_field(name='End Date', value=end_date, inline=True)
            emb.add_field(name='Chapter Count', value=chapter_count, inline=True)
            emb.set_image(url = image)
            await ctx.send(embed = emb)
    ...
    async def char(self, ctx, *query):
        ...
        if data['count'] == 0:
            emb = discord.Embed(title='Character', description= f'Sorry! No data on {query} found.', color = discord.Color.red())
            await ctx.send(embed=emb)
        else:
            id = id = getlist(data)
